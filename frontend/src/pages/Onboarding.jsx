...
// ─────────────────────────────────────────────────────────────
// First screen. User enters their ID and profile info.
// On submit, redirects to diagnostic or dashboard.
// ─────────────────────────────────────────────────────────────
...
export default function Onboarding() {
  ...
  const [step, setStep] = useState(1); // 1 = ID entry, 2 = profile
  const [inputId, setInputId] = useState('');
  const [form, setForm] = useState({ degree: 'BTech', year: '2nd', interest: 'python' });
  const [error, setError] = useState('');

  function handleIdSubmit() {
    const trimmed = inputId.trim();
    if (!trimmed) { setError('Please enter a user ID.'); return; }
    setError('');
    setUserId(trimmed);
    setStep(2);
  }
      ...
      <div className="onboard-content">
        ...
        <div className="onboard-header">
          ...
          <p className="onboard-subtitle">
            AI-powered adaptive learning — personalised to your pace, goals, and gaps.
          </p>
        </div>
        ...
        <div className="onboard-card">
          ...
          {step === 1 && (
            <div className="onboard-step">
              ...
              <p className="step-desc">Enter your student ID or any unique name to get started.</p>
              ...
              <Button onClick={handleIdSubmit} className="btn-full" size="lg">
                Continue →
              </Button>
            </div>
          )}
          ...
          {step === 2 && (
            <div className="onboard-step">
              ...
              <p className="step-desc">This helps the AI personalise recommendations for you.</p>
              ...
              <div className="form-group">
                ...
                <div className="chip-group">
                  {DEGREES.map(d => (
                    <button
                      key={d}
                      className={`chip ${form.degree === d ? 'chip-active' : ''}`}
              ...
              <div className="form-group">
                ...
                <div className="chip-group">
                  {INTERESTS.map(i => (
                    <button
                      key={i}
                      className={`chip ${form.interest === i ? 'chip-active' : ''}`}
